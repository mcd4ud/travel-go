...
TMS.Rental = (() => {
  ...
  function renderList() {
    ...
    <div class="modal-overlay" id="rentalModal">
      <div class="modal modal-lg"><div class="modal-header"><span class="modal-title" id="rentalModalTitle">Buat Voucher Rental Mobil</span><button class="modal-close" onclick="TMS.Rental.closeForm()">✕</button></div>
  ...
  function renderForm(data = {}) {
    ...
    <form id="rentalForm" onsubmit="TMS.Rental.save(event)">
      ...
      <div class="form-row-3">
        <div class="form-group"><label class="form-label">Nama Penyewa *</label><input class="form-control" name="renterName" id="r_name" required placeholder="Nama lengkap"></div>
        <div class="form-group"><label class="form-label">Email</label><input class="form-control" type="email" name="renterEmail" id="r_email" placeholder="email@domain.com"></div>
        <div class="form-group"><label class="form-label">Telepon</label><input class="form-control" name="renterPhone" id="r_phone" placeholder="08xx-xxxx-xxxx"></div>
      </div>
      <div class="form-group"><label class="form-label">No. SIM *</label><input class="form-control" name="licenseNumber" required placeholder="No. SIM"></div>
      ...
      <div class="form-row-3">
        <div class="form-group"><label class="form-label">Tipe Kendaraan *</label>
          <select class="form-control" name="vehicleType" required>
            ...
            <option>SUV</option><option>MPV</option><option>Sedan</option><option>Hatchback</option><option>City Car</option><option>Pick Up</option><option>Bus Mini</option>
          </select>
        </div>
        <div class="form-group"><label class="form-label">Nama Kendaraan *</label><input class="form-control" name="vehicleName" required placeholder="Toyota Avanza 2023"></div>
        <div class="form-group"><label class="form-label">Plat Nomor</label><input class="form-control" name="licensePlate" placeholder="B 1234 XYZ"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Termasuk Supir?</label>
          <select class="form-control" name="withDriver">
            <option value="Tanpa Supir">Lepas Kunci (Tanpa Supir)</option>
            <option value="Dengan Supir">Dengan Supir</option>
          </select>
        </div>
        <div class="form-group"><label class="form-label">Nomor Voucher Rental</label><input class="form-control" name="voucherNumber" placeholder="Contoh: RENT-123456"></div>
      </div>
      <div class="form-group"><label class="form-label">Fasilitas / Catatan Unit</label><input class="form-control" name="facilities" placeholder="Contoh: AC dingin, Full Tank, Bersih"></div>
      <div class="form-section-title"><i data-lucide="map-pin"></i> Jadwal Rental</div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Tanggal Pickup *</label><input class="form-control" type="date" name="pickupDate" required onchange="TMS.Rental.calcDays()"></div>
        <div class="form-group"><label class="form-label">Lokasi Pickup</label><input class="form-control" name="pickupLocation" placeholder="Bandara Soekarno-Hatta"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Tanggal Return *</label><input class="form-control" type="date" name="returnDate" required onchange="TMS.Rental.calcDays()"></div>
        <div class="form-group"><label class="form-label">Lokasi Return</label><input class="form-control" name="returnLocation" placeholder="Bandara Soekarno-Hatta"></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Jumlah Hari</label><div class="form-control" id="daysDisplay" style="color:var(--primary-light);font-weight:700;">0 hari</div><input type="hidden" name="days" id="daysInput" value="0"></div>
      </div>
      ...
      <div class="form-row-3">
        <div class="form-group"><label class="form-label">Harga Modal / Beli (per hari) *</label><div class="input-group"><span class="input-prefix">Rp</span><input class="form-control" type="number" name="costPricePerDay" value="${data.costPricePerDay || ''}" required min="0" placeholder="0" oninput="TMS.Rental.calcTotal()"></div></div>
        <div class="form-group"><label class="form-label">Margin (per hari) *</label><div class="input-group"><span class="input-prefix">Rp</span><input class="form-control" type="number" id="r_marginPerDay" name="marginPerDay" value="${(data.sellingPricePerDay !== undefined && data.costPricePerDay !== undefined) ? (data.sellingPricePerDay - data.costPricePerDay) : ''}" required min="0" placeholder="0" oninput="TMS.Rental.calcTotal()"></div></div>
        <div class="form-group"><label class="form-label">Harga Jual (per hari)</label><div class="input-group"><span class="input-prefix">Rp</span><input class="form-control" type="number" id="r_sellingPricePerDay" name="sellingPricePerDay" value="${data.sellingPricePerDay || ''}" readonly style="background:var(--bg-secondary);" min="0" placeholder="0"></div></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Total Harga Modal</label><div class="form-control" id="rentalTotalCostDisplay" style="color:var(--text-secondary);font-weight:700;">Rp 0</div><input type="hidden" name="costPrice" id="rentalCostPriceHidden"></div>
        <div class="form-group"><label class="form-label">Total Harga Jual</label><div class="form-control" id="rentalTotalSellDisplay" style="color:var(--primary-light);font-weight:700;">Rp 0</div><input type="hidden" name="sellingPrice" id="rentalSellingPriceHidden"></div>
  ...
  function calcTotal() {
    ...
    const costPD = parseFloat(document.querySelector('[name="costPricePerDay"]')?.value) || 0;
    const marginPD = parseFloat(document.querySelector('[name="marginPerDay"]')?.value) || 0;
    const sellPD = costPD + marginPD;

    const sellPDInput = document.getElementById('r_sellingPricePerDay');
    if (sellPDInput) sellPDInput.value = sellPD;
